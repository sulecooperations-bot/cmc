...
export default function WastePage() {
  ...
  const [selectedLayers, setSelectedLayers] = useState<string[]>(["waste"])

  let filteredAssets = mockData.assets.filter((a) => a.type === "waste")
  if (selectedWard) {
    filteredAssets = filteredAssets.filter((a) => a.ward === selectedWard)
  }

  const handleRowClick = (asset: Asset) => {
    router.push(`/assets/waste/${asset.id}`)
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold">Waste Points</h1>
          <p className="text-muted-foreground">Solid waste collection points</p>
        </div>
        <Button>Add Waste Point</Button>
      </div>
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2">
          <DataTable
            columns={columns}
            data={filteredAssets}
            searchPlaceholder="Search waste points..."
